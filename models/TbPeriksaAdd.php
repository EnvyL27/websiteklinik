 ...
 * Page class
 */
class TbPeriksaAdd extends TbPeriksa
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TbPeriksaAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("tbperiksalist"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "tbperiksalist") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "tbperiksaview") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->diagnosa->CurrentValue = null;
        $this->diagnosa->OldValue = $this->diagnosa->CurrentValue;
        $this->terapi->CurrentValue = null;
        $this->terapi->OldValue = $this->terapi->CurrentValue;
        $this->hasilPeriksa->CurrentValue = null;
        $this->hasilPeriksa->OldValue = $this->hasilPeriksa->CurrentValue;
        $this->tarif->CurrentValue = null;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'diagnosa' first before field var 'x_diagnosa'
        $val = $CurrentForm->hasValue("diagnosa") ? $CurrentForm->getValue("diagnosa") : $CurrentForm->getValue("x_diagnosa");
        ...
        // Check field name 'hasilPeriksa' first before field var 'x_hasilPeriksa'
        $val = $CurrentForm->hasValue("hasilPeriksa") ? $CurrentForm->getValue("hasilPeriksa") : $CurrentForm->getValue("x_hasilPeriksa");
        if (!$this->hasilPeriksa->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->hasilPeriksa->Visible = false; // Disable update for API request
            } else {
                $this->hasilPeriksa->setFormValue($val);
            }
        }

        // Check field name 'tarif' first before field var 'x_tarif'
        $val = $CurrentForm->hasValue("tarif") ? $CurrentForm->getValue("tarif") : $CurrentForm->getValue("x_tarif");
        if (!$this->tarif->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tarif->Visible = false; // Disable update for API request
            } else {
                $this->tarif->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->hasilPeriksa->CurrentValue = $this->hasilPeriksa->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['diagnosa'] = $this->diagnosa->CurrentValue;
        $row['terapi'] = $this->terapi->CurrentValue;
        $row['hasilPeriksa'] = $this->hasilPeriksa->CurrentValue;
        $row['tarif'] = $this->tarif->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->id_pasien->ViewValue = $this->id_pasien->Lookup !== null && is_array($this->id_pasien->Lookup->Options) ? $curVal : null;
            }
            if ($this->id_pasien->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = "`id_pasien`" . SearchString("=", $this->id_pasien->CurrentValue, DATATYPE_NUMBER, "");
                }
            ...
            $this->diagnosa->EditValue = HtmlEncode($this->diagnosa->CurrentValue);
            ...
            $this->hasilPeriksa->EditValue = HtmlEncode($this->hasilPeriksa->CurrentValue);
            ...
            $this->tarif->EditValue = HtmlEncode($this->tarif->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->diagnosa->Required) {
            if (!$this->diagnosa->IsDetailKey && EmptyValue($this->diagnosa->FormValue)) {
        ...
        if ($this->terapi->Required) {
            if (!$this->terapi->IsDetailKey && EmptyValue($this->terapi->FormValue)) {
                $this->terapi->addErrorMessage(str_replace("%s", $this->terapi->caption(), $this->terapi->RequiredErrorMessage));
            }
        }
        if ($this->hasilPeriksa->Required) {
            if (!$this->hasilPeriksa->IsDetailKey && EmptyValue($this->hasilPeriksa->FormValue)) {
                $this->hasilPeriksa->addErrorMessage(str_replace("%s", $this->hasilPeriksa->caption(), $this->hasilPeriksa->RequiredErrorMessage));
            }
        }
        if ($this->tarif->Required) {
            if (!$this->tarif->IsDetailKey && EmptyValue($this->tarif->FormValue)) {
                $this->tarif->addErrorMessage(str_replace("%s", $this->tarif->caption(), $this->tarif->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->tarif->FormValue)) {
            $this->tarif->addErrorMessage($this->tarif->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // id_pasien
        $this->id_pasien->setDbValueDef($rsnew, $this->id_pasien->CurrentValue, 0, false);

        // tanggal
        $this->tanggal->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal->CurrentValue, 0), null, false);

        // keluhan
        $this->keluhan->setDbValueDef($rsnew, $this->keluhan->CurrentValue, "", false);

        // diagnosa
        $this->diagnosa->setDbValueDef($rsnew, $this->diagnosa->CurrentValue, "", false);

        // terapi
        $this->terapi->setDbValueDef($rsnew, $this->terapi->CurrentValue, "", false);

        // hasilPeriksa
        $this->hasilPeriksa->setDbValueDef($rsnew, $this->hasilPeriksa->CurrentValue, "", false);

        // tarif
        $this->tarif->setDbValueDef($rsnew, $this->tarif->CurrentValue, 0, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("tbperiksalist"), "", $this->TableVar, true);
